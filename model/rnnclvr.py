...
class RNN_CLVR(pl.LightningModule):
    ...
    def __init__(self, feature_extractor, aggregation="cat", queue_size=65536, 
                        num_view=2, shift_range=1, focal=True):
        super().__init__()
        ...
        self.relation_module = nn.Sequential(collections.OrderedDict([
          ...
          ("relu",     nn.LeakyReLU()),
          ("linear2",  nn.Linear(256, 1)),   # output unbounded logits (batch_size x logit)
        ]))
    ...
    def nn_relation_estimation(self, Z, P):
        ...
        nn_z1 = self.cos_find_nn(z1)  # (32, 3, 32, 32)
        nn_z2 = self.cos_find_nn(z2)  # (32, 3, 32, 32)
        logit_lst1 = list()
        logit_lst2 = list()
        batch_size = z1.shape[0] #self.shft_rng if self.shft_rng else z1.shape[0]
    ...
    def training_step(self, batch, batch_idx):
        ...
        for x in X:  # perform forward function to each view (defualt : 2 view)
            out = self(x, target)  # diff view have same target
            tmp_outs.append(out)
        # merge all outputs according to the same key
        outs = {k: [out[k] for out in tmp_outs] for k in tmp_outs[0].keys()}
        
        outs["loss"] = sum(outs["loss"]) / self.num_view
        outs["acc1"] = sum(outs["acc1"]) / self.num_view
        outs["acc5"] = sum(outs["acc5"]) / self.num_view
        metrics = {
        ...
        logit_lst1, logit_lst2 = \
                        self.nn_relation_estimation(outs['z'], outs['p'])
        
        prb_v1, prb_v2 = self.logit2prob(logit_lst1), self.logit2prob(logit_lst2)
        target = torch.zeros([prb_v1.shape[0]], dtype=torch.long).to('cuda') # to device is needed, lighting bug.. 
        
        # BYOL style symmetric loss, done!
        total_loss = self.loss_fn(prb_v1, target) / 2 \
                        + self.loss_fn(prb_v2, target) / 2

        # does not support evaluation of nn_support set acc
        self.dequeue_and_enqueue(outs['z'][0])
